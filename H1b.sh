...
{
    NORMAL=`echo "\033[m"`
    MENU=`echo "\033[1;36m"` #Red
    NUMBER=`echo "\033[1;33m"` #yellow
    FGRED=`echo "\033[41m"`
    RED_TEXT=`echo "\033[31m"`
    ENTER_LINE=`echo "\033[33m"`
    echo -e "${MENU}\e[1m\e[47m\e[1;35m------------------------------[H1B DashBoard]---------------------------------${NORMAL}"
echo -e "${NUMBER} [Use the following options to perform the operation on H1B dataset]${NUMBER}"
...
echo -e "${MENU}${NUMBER} 15)${MENU} Export result for question no 10 to MySql database.${NORMAL}"
    ...
    echo -e "${NUMBER}Please enter a menu option and enter or ${RED_TEXT}enter to exit. ${NUMBER}"
    read opt
}
...
while [ opt != '' ]
    ...
    if [[ $opt = "" ]]; then 
 	...
 	hadoop fs -cat /h1b/output/ans1a/p*;
	...
	hadoop fs -cat /h1b/output/ans1b/p*;
...
8) clear;
 ...
 13) clear;
	...
	hadoop fs -cat /h1b/output/ans9/p*;
 	show_menu;
        ;;
14) clear;
	hadoop fs -rmr /h1b/output/ans10;
	hadoop jar h1b.jar Ques10 /user/hive/warehouse/h1b.db/h1b_final /h1b/output/ans10;
	hadoop fs -cat /h1b/output/ans10/p*;
 	show_menu;
        ;;
15) clear;
        option_picked "Now lets export the Ans10 to MySql";
	sqoop export --connect jdbc:mysql://localhost/h1b --username root --password '1231' --table exportques10 --export-dir /h1b/output/ans10 --input-fields-terminated-by '\t' ;
show_menu;
;;
